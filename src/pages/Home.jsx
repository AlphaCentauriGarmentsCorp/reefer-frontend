...
const MANIFESTO =
  "Reefer was never a plan. It was a dare. We print tees the way waves hit: loud, sudden, and gone before you can screenshot them. Small batches. Printed by hand. No committees deciding what you wear. If you missed the last drop, that was the universe telling you to move faster. Paddle out.";
...
// Document order. The rail's active index is a position in this list, so it has to
// match the order the sections appear in the JSX below — not their names.
const CHAPTERS = [
  { href: "#hero", label: "//00 WAVE" },
  { href: "#drop", label: "//01 THE DROP" },
...
export default function Home() {
  ...
  useEffect(() => {
    ...
    const links = railRef.current
      ...
      : [];
    // Document order, same as CHAPTERS — the rail lights link i when sections[i] is
    // the last one past the halfway line, so a mismatch here lights the wrong chapter.
    const sections = [heroRef.current, dropRef.current, manifestoRef.current, cultRef.current];
    const paras = [
      [heroContentRef.current, 0.3],
      [badgeRef.current, 0.15],
    ].filter(([el]) => el);
    // Matched by rect containment rather than by index, so order is not load-bearing
    // here — kept in document order anyway so it does not read as a mistake.
    const bands = [
      [heroRef.current, "light"],
      [dropRef.current, "orange"],
      [manifestoRef.current, "dark"],
      [cultRef.current, "light"],
  ...
  return (
    <div style={{ background: "#F6F1E7", color: "#101010", minHeight: "100vh", overflowX: "clip" }}>
      ...
      >
        ...
        >
          <div className="rf-hero-eyebrow" style={{ display: "flex", alignItems: "center", gap: 12, marginBottom: 18, animation: "fadeUp 0.7s both" }}>
            <span
              style={{
                background: "#F97B0C",
                color: "#101010",
          ...
          <div className="rf-hero-ctarow" style={{ display: "flex", alignItems: "center", gap: 22, marginTop: 34, animation: "fadeUp 0.8s 0.5s both" }}>
            <Link
              to="/products"
              className="rf-cta"
              style={{
        ...
        {/* spinning badge */}
        <div ref={badgeRef} data-para="0.15" className="rf-hero-badge" style={{ position: "absolute", top: 118, right: "clamp(180px, 17vw, 320px)", zIndex: 1, width: 150, height: 150, animation: "fadeUp 1s 0.6s both" }}>
          ...
          <svg viewBox="0 0 150 150" style={{ position: "absolute", inset: 0, width: "100%", height: "100%", animation: "spin 14s linear infinite" }}>
            ...
            <text style={{ fontFamily: "Archivo, sans-serif", fontWeight: 900, fontSize: "12.5px", letterSpacing: "3px", fill: "#101010" }}>
              <textPath href="#circ">REEFER MNL ★ NEWLY DROPPED ★ SMALL BATCH ★ RIDE THE WAVE ★</textPath>
            </text>
          </svg>
        </div>
      ...
      {/* ===== //01 The Drop =====
          The 1440 measure moved off the section and onto an inner wrapper. Left where
          it was, the orange would have painted a 1440px-wide block with cream gutters
          either side of it instead of a full-bleed band. */}
      <section id="drop" ref={dropRef} data-chapter="1" style={{ background: "#F97B0C", color: "#101010", padding: "130px 0 100px", borderTop: "2px solid #101010" }}>
        <div style={{ maxWidth: 1440, margin: "0 auto", padding: "0 32px" }}>
          <div style={{ display: "flex", flexDirection: "column", gap: 10, marginBottom: 54 }}>
            {/* Ink, not orange: the eyebrow's usual #F97B0C is now the background. */}
            <span style={{ fontWeight: 900, fontSize: 12, letterSpacing: "0.22em", color: "#101010" }}>//01 THE DROP</span>
            <div style={{ display: "flex", flexWrap: "wrap", alignItems: "baseline", justifyContent: "space-between", gap: 16 }}>
              <h2 style={{ fontFamily: "Anton, sans-serif", fontWeight: 400, fontSize: "clamp(48px, 7vw, 110px)", margin: 0, textTransform: "uppercase", lineHeight: 0.9 }}>
                Newly dropped<span style={{ color: "#F6F1E7" }}>.</span>
              </h2>
              {/* Held under full ink so it still reads as secondary — #6B6357 was tuned
                  for cream and goes muddy on orange. */}
              <span style={{ fontWeight: 700, fontSize: 12, letterSpacing: "0.16em", color: "rgba(16,16,16,0.7)" }}>
      ...
      {/* ===== //02 Manifesto ===== */}
      <section id="manifesto" ref={manifestoRef} data-chapter="2" style={{ background: "#101010", color: "#F6F1E7", padding: "150px 32px", position: "relative", overflow: "hidden" }}>
        <span
          style={{
            position: "absolute",
            top: -40,
        ...
        <div style={{ maxWidth: 1100, margin: "0 auto", display: "flex", flexDirection: "column", gap: 40, position: "relative" }}>
          <span style={{ fontWeight: 900, fontSize: 12, letterSpacing: "0.22em", color: "#F97B0C" }}>//02 MANIFESTO</span>
      ...
      {/* ===== //03 The Cult ===== */}
      <section id="cult" ref={cultRef} data-chapter="3" style={{ background: "#F6F1E7", color: "#101010", padding: "130px 0 110px", overflow: "hidden", borderTop: "2px solid #101010" }}>
        <div style={{ display: "flex", flexDirection: "column", gap: 56 }}>
          <div style={{ maxWidth: 1100, margin: "0 auto", padding: "0 32px", display: "flex", flexDirection: "column", gap: 20, width: "100%" }}>
            {/* Back to the orange eyebrow every other band uses — it was ink here only
                because the band itself used to be orange. */}
            <span style={{ fontWeight: 900, fontSize: 12, letterSpacing: "0.22em", color: "#F97B0C" }}>//03 THE CULT</span>
            <h2 style={{ fontFamily: "Anton, sans-serif", fontWeight: 400, fontSize: "clamp(44px, 6.5vw, 96px)", lineHeight: 0.9, margin: 0, textTransform: "uppercase" }}>
              ...
              <br />
              {/* Was cream, which is now the background — orange is the second-line
                  contrast on a light band. */}
              <span style={{ color: "#F97B0C" }}>and warm intros.</span>
            </h2>
            ...
            <div style={{ display: "flex", gap: 16, marginTop: 10 }}>
              {/* Orange drop-shadow: the cream one was only visible because it sat on
                  orange, and would vanish into the band now. */}
              <a href="#" className="rf-cta" style={{ fontFamily: "Anton, sans-serif", fontSize: 15, letterSpacing: "0.1em", background: "#101010", color: "#F6F1E7", textDecoration: "none", padding: "14px 26px", border: "2px solid #101010", boxShadow: "5px 5px 0 #F97B0C", display: "inline-block" }}>
                INSTAGRAM
              </a>
          ...
          <div ref={skewRef} style={{ overflow: "hidden", borderTop: "2px solid #101010", borderBottom: "2px solid #101010", background: "#F97B0C", padding: "10px 0", willChange: "transform" }}>
            <div style={{ display: "flex", width: "max-content", animation: "marquee 16s linear infinite", fontFamily: "Anton, sans-serif", fontSize: "clamp(60px, 10vw, 150px)", lineHeight: 1, color: "transparent", WebkitTextStroke: "2px #101010", textTransform: "uppercase" }}>
              <span style={{ whiteSpace: "nowrap", paddingRight: 48 }}>Ride the wave ★ Small batch ★ Ride the wave ★ Small batch ★&nbsp;</span>
              <span style={{ whiteSpace: "nowrap", paddingRight: 48 }}>Ride the wave ★ Small batch ★ Ride the wave ★ Small batch ★&nbsp;</span>
            </div>
          </div>
        </div>
